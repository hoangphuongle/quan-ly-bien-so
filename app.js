...
async function initApp() {
    ...
    try {
        ...
        if (error && error.code !== 'PGRST116') {
            ...
            console.log("Di cư dữ liệu từ Local Storage lên Supabase...");
            loadLocalFallback();
            await saveState(); // Push to cloud
        }
    } catch (e) {
        console.error("Lỗi:", e);
        loadLocalFallback();
...
// AUTO INJECT DATA FROM USER IMAGE
setTimeout(() => {
    if (!localStorage.getItem('injected_hao_refund')) {
        let hasLoc = state.records.some(r => r.customerName === 'Lê Thị Lộc');
        if (!hasLoc) {
            state.records.push({
                id: 'rec_' + Date.now(),
                customerName: 'Lê Thị Lộc',
                carPrice: 0,
                carPaymentMethod: 'cash',
                plateFee: 545000,
                ...
                hasTaxCode: true,
                stage: 2, 
                taxPaidBy: 'staff', fee105kPaidBy: null, fee100kPaidBy: null, staffReimbursed: 0,
                taxDate: new Date().toISOString().split('T')[0],
                pressPlateDate: '', sendServiceDate: '', callCustomerDate: '',
                deliverCarDate: '', plateNumber: '', promisePlateDate: '', regNote: '',
                plateNote: '', receivePlateDate: '', deliverPlateDate: '', deliverStaff: ''
            });
        }
        ...
        let hasKeo = state.expenses.some(e => e.amount === 517000);
        if (!hasKeo) {
            state.expenses.push({
                id: 'exp_1_' + Date.now(),
                type: 'chi',
                amount: 517000,
                reason: 'Mua keo, bút xoá, ổ điện',
                payer: 'staff',
                date: new Date().toISOString()
            });
        }
        
        let hasHa = state.expenses.some(e => e.amount === 340000);
        if (!hasHa) {
            state.expenses.push({
                id: 'exp_2_' + (Date.now()+1),
                type: 'chi',
                amount: 340000,
                reason: 'Ứng tiền thuế khách Hà',
                payer: 'staff',
                date: new Date().toISOString()
            });
        }

        let hasHaoRefund = state.expenses.some(e => e.amount === 378000 && e.type === 'thu');
        if (!hasHaoRefund) {
            state.expenses.push({
                id: 'exp_3_' + (Date.now()+2),
                type: 'thu',
                amount: 378000,
                reason: 'Chủ (Thuý Oanh) CK trả tiền ứng khách Hảo',
                payer: 'staff',
                date: new Date().toISOString()
            });
        }
        ...
        saveState();
        localStorage.setItem('injected_hao_refund', 'true');
    }
        
    // INJECT 5 CUSTOMERS (Lộc, Thiên, Dương, Hảo, Trâm)
    if (!localStorage.getItem('injected_5_customers')) {
        const customers = [
            { name: 'Lê Thị Lộc', tax: 340000, plate: 105000, police: 100000 },
            { name: 'Lê Quang Thiên', tax: 378000, plate: 105000, police: 100000 },
            { name: 'Nguyễn Thùy Dương', tax: 378000, plate: 105000, police: 100000 },
            { name: 'Phạm Trần Phú Hảo', tax: 378000, plate: 105000, police: 100000 },
            { name: 'Hoàng Khánh Trâm', tax: 480000, plate: 105000, police: 100000 }
        ];

        let changed = false;
        customers.forEach((c, idx) => {
            let existing = state.records.find(r => r.customerName.toLowerCase().includes(c.name.toLowerCase()));
            if (!existing) {
                state.records.push({
                    id: 'rec_5c_' + Date.now() + '_' + idx,
                    customerName: c.name,
                    carPrice: 0,
                    carPaymentMethod: 'cash',
                    plateFee: c.tax + c.plate + c.police, // Giả sử cửa hàng đưa đủ tiền phí
                    platePaymentMethod: 'cash',
                    actualCost: c.tax + c.plate + c.police,
                    taxCost: c.tax,
                    plateCost: c.plate,
                    policeCost: c.police,
                    hasTaxCode: true,
                    stage: 2, 
                    taxPaidBy: 'staff', fee105kPaidBy: null, fee100kPaidBy: null, staffReimbursed: 0,
                    payTaxDate: new Date().toISOString().split('T')[0],
                    pressPlateDate: '', sendServiceDate: '', callCustomerDate: '',
                    deliverCarDate: '', plateNumber: '', promisePlateDate: '', regNote: '',
                    plateNote: '', receivePlateDate: '', deliverPlateDate: '', deliverStaff: ''
                });
                ...
                if (existing.platePaymentMethod !== 'cash') {
                    existing.platePaymentMethod = 'cash';
                    existing.taxCost = c.tax;
                    existing.plateCost = c.plate;
                    existing.policeCost = c.police;
                    // Bổ sung phí nếu chưa có phí
                    if (!existing.plateFee) {
                        existing.plateFee = c.tax + c.plate + c.police;
                    }
                    changed = true;
                }
            }
        ...
        localStorage.setItem('injected_5_customers_v2', 'true');
    }

    // INJECT 8.8 TRANSACTIONS
    if (!localStorage.getItem('injected_88_transactions')) {
        const txs = [
            { type: 'thu', amount: 70000, reason: 'Thu vá lốp sau DV', payer: 'fund' },
            { type: 'thu', amount: 120000, reason: 'Thu nguyễn bàng', payer: 'fund' },
            { type: 'thu', amount: 120000, reason: 'Biển số khách dung', payer: 'fund' },
            { type: 'thu', amount: 800000, reason: 'Tiền mặt bỏ két tiền bs xe đỗ công vượng', payer: 'fund' },
            { type: 'thu', amount: 750000, reason: 'Thay lốp', payer: 'fund' }
        ];
        ...
        saveState();
        localStorage.setItem('injected_88_transactions', 'true');
        window.location.reload();
    }
}, 500);
